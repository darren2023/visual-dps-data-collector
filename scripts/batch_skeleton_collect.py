#!/usr/bin/env python3
"""递归扫描文件夹内全部视频，仅采集骨架（不算碰撞），效果对齐 Web「仅骨架 + 文件夹批处理」。

用法:
  python scripts/batch_skeleton_collect.py D:/videos/1-1-1 --camera-label 1-1-1
  python scripts/batch_skeleton_collect.py D:/videos --camera-label 1-6-2 --save-video
...
说明:
  - 递归包含所有子文件夹中的视频
  - 不读取标注 JSON，不写碰撞事件（meta.collision_computed=false）
  - 结果写入 localdata/json/{机位slug}/{视频主名}_{backend}/
  - --group-by-subfolder：用 root 下第一级子目录名作机位（多机位目录批量时用）
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    for video_path, rel in videos:
        ...
        if not cam:
            ...
            return 2
        grouped[cam].append((video_path, rel))

    flat: list[tuple[Path, str, str, str]] = []
    slug_by_label: dict[str, str] = {}
    for cam_label in sorted(grouped.keys()):
        slug_by_label[cam_label] = allocate_camera_storage_slug(paths, cam_label)
        for video_path, rel in grouped[cam_label]:
            flat.append((video_path, rel, cam_label, slug_by_label[cam_label]))
    ...
    if args.group_by_subfolder:
        print(f"📂 机位分组: 按第一级子目录（{len(slug_by_label)} 个机位）")
    else:
        print(f"📂 机位: {fallback_camera} → {slug_by_label.get(fallback_camera, '')}")
    ...
    collect_config = build_collect_config_snapshot(
        ...
        camera_slug=slug_by_label.get(fallback_camera) if fallback_camera else None,
        batch_id=batch_id,
        annotation_source="skeleton_only",
        skeleton_only=True,
    )
    ...
    try:
        for cam_label, cam_slug in slug_by_label.items():
